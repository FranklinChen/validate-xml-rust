...
/// Progress update for validation workflow
#[derive(Debug, Clone)]
...
pub struct ValidationConfig {
    ...
    /// Timeout for individual file validation
    pub validation_timeout: Duration,
    /// Whether to fail fast on first error
    pub fail_fast: bool,
    /// Whether to show progress during validation
...
impl ValidationEngine {
    ...
    /// Validate all XML files in a directory using fully async operations with comprehensive workflow
    pub async fn validate_directory(
        &self,
        directory: &Path,
        file_discovery: &FileDiscovery,
    ) -> Result<ValidationResults> {
        self.validate_directory_with_progress(directory, file_discovery, None)
            .await
    }

    /// Validate all XML files in a directory with progress tracking
    pub async fn validate_directory_with_progress(
        &self,
        directory: &Path,
        file_discovery: &FileDiscovery,
        ...
        let files = file_discovery.discover_files(directory).await?;
        ...
        let _aggregation_start = Instant::now();
        if let Some(ref callback) = progress_callback {
            callback(ValidationProgress {
                current_file: None,
                ...
                elapsed: workflow_start.elapsed(),
                estimated_remaining: Some(Duration::from_millis(50)), // Estimate
            });
        }
        ...
        if let Some(ref callback) = progress_callback {
            callback(ValidationProgress {
                ...
                elapsed: final_results.total_duration,
    ...
    pub async fn validate_files_with_progress(
        ...
        // Create progress tracking channel if callback is provided
        let (progress_tx, mut progress_rx) = if progress_callback.is_some() {
            let (tx, rx) = mpsc::unbounded_channel::<(PathBuf, bool)>();
            (Some(tx), Some(rx))
        } else {
            (None, None)
        };
        ...
        let progress_task = if let (Some(callback), Some(mut rx)) =
            (progress_callback.clone(), progress_rx.take())
        {
            Some(tokio::spawn(async move {
                let mut completed = 0;
                let mut current_file = None;

                while let Some((file_path, is_complete)) = rx.recv().await {
                    if is_complete {
                        completed += 1;
                    } else {
    ...
    async fn validate_single_file_internal(
        ...
        // Step 1: Load schema for the file (fully async)
        let cached_schema = match schema_loader.load_schema_for_file(&file_path).await {
            Ok(schema) => schema,
            ...
            Err(ValidationError::SchemaNotFound { url }) => {
                let duration = start_time.elapsed();
            ...
            Err(e) => {
                let duration = start_time.elapsed();
                return FileValidationResult::error(file_path, e, duration);
            }
        };

        // Step 2: Parse schema using libxml2 (synchronous, thread-safe operation)
        let schema_ptr = match libxml2_wrapper.parse_schema_from_memory(cached_schema.data.as_ref()) {
            Ok(ptr) => ptr,
            Err(e) => {
                let duration = start_time.elapsed();
                return FileValidationResult::error(
                    file_path,
                    e.into(), // Convert LibXml2Error to ValidationError
                    duration,
                );
            }
        };

        // Step 3: Validate file against schema (synchronous, thread-safe operation)
        let validation_result = match libxml2_wrapper.validate_file(&schema_ptr, &file_path) {
            Ok(result) => result,
            Err(e) => {
                let duration = start_time.elapsed();
                return FileValidationResult::error(
        ...
        let duration = start_time.elapsed();
        let schema_url = cached_schema.metadata.url.clone();

        // Convert libxml2 result to file validation result
        match validation_result {
            ValidationResult::Valid => FileValidationResult::valid(file_path, schema_url, duration),
            ValidationResult::Invalid { error_count } => {
                FileValidationResult::invalid(file_path, schema_url, error_count, duration)
            }
            ValidationResult::InternalError { code } => FileValidationResult::error(
                file_path,
                ValidationError::Config(format!("LibXML2 internal error: {}", code)),
                duration,
            ),
        }
    }
    ...
    pub async fn run_comprehensive_validation(
        ...
        self.validate_directory_with_progress(directory, file_discovery, progress_callback)
            .await
    }
}
...
<root>content</root>"#;
    ...
    async fn test_validation_status_from_validation_result() {
        ...
        let invalid_result = ValidationResult::Invalid { error_count: 3 };
