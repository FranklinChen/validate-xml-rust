//! Comprehensive test suite for XML Validator
//!
...
async fn test_schema_cache_basic_operations() {
    ...
    // Set and get from unified cache (uses both memory and disk)
    cache.set(key, test_data.clone(), None, None).await.unwrap();
    let retrieved = cache.get(key).await.unwrap();
    assert!(retrieved.is_some());
    assert_eq!(retrieved.unwrap().data.to_vec(), test_data);
}
...
#[tokio::test]
#[ignore] // Performance test - results vary by machine/load - run with: cargo test -- --ignored
async fn test_cache_performance() {
    ...
    for i in 0..iterations {
        ...
        let _ = cache.set(&key, test_data.clone(), None, None).await;
    }
    ...
    // Cache operations should be reasonably fast (machine-dependent)
    assert!(
    ...
    assert!(
        ...
        "Cache reads not faster than writes"
    );
}
...
<root>Content {}</root>"#,
        ...
        fs::write(temp_dir.path().join(format!("test_{}.xml", i)), xml_content)
            .await
            .unwrap();
    }
    ...
    // Run sequential operations to test basic functionality
    let timer = PerformanceTimer::new();
    let files = discovery.discover_files(temp_dir.path()).await.unwrap();
    let elapsed = timer.elapsed();
    ...
    println!("File discovery completed in: {:?}", elapsed);
...
async fn test_comprehensive_performance_suite() {
    ...
    // Run performance tests (call them directly since they're in the same module)
